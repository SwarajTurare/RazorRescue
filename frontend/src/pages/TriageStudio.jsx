...
export default function TriageStudio() {
  ...
  const [simulateQuietHours, setSimulateQuietHours] = useState(false);
  ...
  const transaction = useMemo(() => {
    return transactions.find((item) => item.transaction_id === transactionId);
  ...
  useEffect(() => {
    if (transaction) {
      setPhone(String(transaction.phone || ""));
    }
  }, [transaction]);
  ...
  async function runRecovery() {
    ...
    try {
      ...
      const response = await api.triage({
        ...
        forceDispatch: false,
        forceApprove,
        simulatedHour: simulateQuietHours ? 23 : null,
      });
      ...
      if (status.includes("ready")) {
        toast.success("Recovery workflow prepared.");
      } else {
        toast.info("Workflow gate applied.");
      }
    } catch (error) {
      toast.error(error?.message || "Recovery pipeline failed.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function refreshCommunication({
    ...
    const cleanPhone = String(nextPhone || "").replace(/\D/g, "");
    ...
    try {
      const response = await api.communication({
        transaction: {
          ...transaction,
          phone: cleanPhone.slice(-10),
        },
        ...
        effectiveAmount: Number(
          result.effective_amount ?? transaction.amount ?? 0,
        ),

        decision: result.decision || {},

        failureReason: result.failure_reason || transaction.failure_reason,
      ...
      toast.success(`Communication updated for ${nextLanguage}.`);
    } catch (error) {
      toast.error(error?.message || "Unable to update customer communication.");
    } finally {
  ...
  async function generateVoice() {
    const message = result?.spoken_message || result?.final_message || "";

    if (!message.trim()) {
      toast.error("No customer message is available for voice generation.");
      return;
    }
    ...
    try {
      ...
      if (!response?.ok || !response?.audio_base64) {
        throw new Error(
          response?.message || "Voice service did not return audio.",
        );
      }

      const bytes = Uint8Array.from(atob(response.audio_base64), (character) =>
        character.charCodeAt(0),
      );

      const blob = new Blob([bytes], {
        type: response.mime_type || "audio/mpeg",
      });
      ...
      toast.success(`${language} voice note generated.`);
    } catch (error) {
      toast.error(error?.message || `Unable to generate ${language} voice.`);
    } finally {
  ...
  async function copyDraft() {
    const message = result?.final_message || result?.spoken_message || "";

    if (!message.trim()) {
      toast.error("No recovery message is available.");
      return;
    }
  ...
  function getStatusTone() {
    ...
    if (status.includes("ready") || status.includes("recovered")) {
      return "success";
    }
  ...
  const customerMessage = result?.final_message || result?.spoken_message || "";
  ...
  const paymentUrl = useMemo(() => {
    const url = result?.retry_url || "";

    if (url && !url.includes("rzp.io/i/TXN_")) {
      return url;
    }

    if (transaction?.transaction_id) {
      return `${window.location.origin}/recovery-demo/${encodeURIComponent(
    ...
    return "";
  }, [result?.retry_url, result?.effective_amount, transaction, phone]);
  ...
  const whatsappUrl =
    cleanPhone.length >= 10 && customerMessage
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
        ...
        <section className="rr-surface rr-glow rounded-2xl p-4 md:p-5">
          <div className="flex items-center gap-2">
            <Bot size={17} className="text-rr-gold" />
          ...
          <div className="mt-5 space-y-4">
                  ...
                  transactions.map((item) => (
                    <option
                      ...
                      className="
                        ...
                        "
                    >
                      {item.transaction_id} — {item.name} — ₹{item.amount}
                    </option>
                  ))
                )}
              </select>
            ...
            {transaction && (
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  ["Gateway code", transaction.raw_gateway_error],
                  ["Root cause", transaction.failure_reason],
                  ["Category", transaction.category],
                  ["Attempts", transaction.attempt_number],
                  ["Aging", `${transaction.days_overdue} days`],
                  [
                    "Gross margin",
                    `${Math.round(
                      Number(transaction.gross_margin || 0) * 100,
                    )}%`,
                  ],
                ].map(([label, value]) => (
                  <div
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="block text-xs text-rr-muted">
                ...
                <div className="mt-1 text-[10px] text-rr-dim">
                  The selected transaction number is used initially. You can
                  edit it.
                </div>
              </label>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <label className="flex items-center gap-2 text-xs text-rr-muted">
                <input
                  type="checkbox"
                  checked={simulateQuietHours}
                  onChange={(event) =>
                    setSimulateQuietHours(event.target.checked)
                  }
              ...
              <label className="flex items-center gap-2 text-xs text-rr-muted">
                <input
                  ...
                  onChange={(event) => setForceApprove(event.target.checked)}
                  className="accent-[#D9A353]"
                />
                Force approve HITL
              </label>
        ...
        <section className="rr-surface rr-glow rounded-2xl p-4 md:p-5">
          <div className="flex items-center gap-2">
            <ShieldAlert size={17} className="text-rr-gold" />
          ...
          {!result ? (
            <div className="grid min-h-[360px] place-items-center px-4 text-center text-sm text-rr-dim">
              Run the workflow to inspect the decision, localized message, voice
              and payment QR.
            </div>
          ) : (
            <div className="mt-5 space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <StatusBadge tone={getStatusTone()}>
                  {result.status || "Workflow completed"}
                </StatusBadge>
              ...
              <div className="rounded-xl border border-rr-border/70 bg-rr-bg p-4">
                ...
                <div className="mt-2 text-sm font-medium text-rr-text">
                  {result.decision?.action || "No intervention specified"}
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-xl border border-rr-border/70 bg-rr-bg p-4">
                  ...
                  <div className="mt-2 text-sm font-medium text-rr-text">
                    {result.decision?.dispatch_timing || "Not dispatched"}
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-rr-gold/20 bg-rr-gold/[0.04] p-4">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <button
                    ...
                    disabled={voiceLoading || !customerMessage}
                    className="
                      inline-flex
                      items-center
                      justify-center
                  ...
                  <a
                    ...
                    rel="noreferrer"
                    onClick={(event) => {
                      if (cleanPhone.length < 10 || !customerMessage) {
                ...
                {voiceUrl && (
                  <div className="mt-4 rounded-xl border border-rr-border bg-rr-bg p-3">
                    <div className="mb-2 flex items-center gap-2 text-xs text-rr-muted">
                      <Volume2 size={14} className="text-rr-gold" />
                ...
                {paymentUrl ? (
                  <div className="mt-4 rounded-xl border border-rr-border bg-rr-bg p-4">
                    <div className="mb-3 flex items-center gap-2">
                      <QrCode size={16} className="text-rr-gold" />
                    ...
                    <div className="flex flex-col items-center gap-4 rounded-xl border border-rr-border/70 bg-[#0E0B08] p-4 sm:flex-row">
                      ...
                      <div className="min-w-0 text-center sm:text-left">
                        ...
                        <p className="mt-1 break-words text-xs leading-5 text-rr-muted">
                          The QR opens the same payment page used by the button.
                        </p>
